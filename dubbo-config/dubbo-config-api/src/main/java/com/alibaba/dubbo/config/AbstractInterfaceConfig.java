...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
        ...
        if (registries != null && !registries.isEmpty()) {
            for (RegistryConfig config : registries) {
                ...
                if (address != null && address.length() > 0
                    ...
                    //若不存在protocol参数，默认'dubbo'添加到map几个中
                    ...
                    for (URL url : urls) {
                        ...
                        if ((provider && url.getParameter(Constants.REGISTER_KEY, true))//服务提供者，如果是只订阅不注册，不添加到结果中
                                || (!provider && url.getParameter(Constants.SUBSCRIBE_KEY, true)))//服务消费者，只注册不订阅，不添加结果
                        {
    ...
    protected URL loadMonitor(URL registryURL) {
        ...
        if (ConfigUtils.isNotEmpty(address)) {
            //若不存在"protocol",默认"dubbo"添加到mao集合中
